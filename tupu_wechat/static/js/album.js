...
(function($) {
    ...
    var template = $('<div class="tp-wrapper tp-album">\
        <section class="tp-store-wrapper clearfix">\
        ...
        <section class="tp-user clearfix">\
            <img class="js-user-avatar" src="" width="90"/>\
            <div class="tp-user-info">\
                <p class="tp-user-name"></p>\
                <p class="fz26">添加在<span class="js-album-name">「」</span></p>\
            </div>\
        </section>\
    ...
    $.get(api + 'api/v1/apps/qiniu/', function(data){
        if (data) {
            qiniu = data;
            $('body').find('.modal-main .logo').attr('src', 'http://dn-' + qiniu.bucket +'.qbox.me/img/logo_big.png');
            $('body').find('.modal-main .info').attr('src', 'http://dn-' + qiniu.bucket +'.qbox.me/img/info.png');
            $('body').find('.tp-album-photo .loading').attr('src', 'https://dn-' + qiniu.bucket +'.qbox.me/img/refresh_web_white.gif');
    		 getAlbum();
        }
    });
     ...
     */
    function getAlbum() {
        var url = api +  'api/v1/shares/album/';
        var data = {
        ...
        $.get(url, data, function(data){
            ...
            var user = data.posts[0].user;
            var group = data.posts[0].groups[0];

            if (user.headimgurl) {
                $('.tp-user img').attr('src', user.headimgurl);
            } else {
	            $('.tp-user img').attr('src', 'https://dn-' + qiniu.bucket +'.qbox.me/img/avatar_default.png');
	        }
            $('.tp-user-name').html(user.nickname);
            $('.js-album-name').html('「' + group.name + '」');
            $('.js-total').html(images_len);
            if (images_len) {
            ...
            wx.onMenuShareTimeline({
                title: '请给' + user.nickname + '的好照片点赞：TA在相册「' + group.name + '」添加的照片', // 分享标题
                link: window.location.href, // 分享链接
                imgUrl: images_len ? data.posts[0].photo.url : 'https://dn-' + qiniu.bucket +'.qbox.me/img/album_blue_square.png', // 分享图标
                success: function () { 
                    // 用户确认分享后执行的回调函数
                },
                cancel: function () { 
            ...
            wx.onMenuShareAppMessage({
                title: '请给' + user.nickname + '的好照片点赞', // 分享标题
                desc: 'TA在相册「' + group.name + '」添加的照片', // 分享描述
                link: window.location.href, // 分享链接
                imgUrl: images_len ? data.posts[0].photo.url : 'https://dn-' + qiniu.bucket +'.qbox.me/img/album_blue_square.png', // 分享图标
    ...
    hammerModalDownload.on('swipeleft', function(e) {
        ...
        if (e.target == $('.modal-download .modal-close')[0]) {
            $('html,body').css({
                'height': 'auto',
                'overflow': 'auto'
            });
    ...
    function closeModal() {
        ...
        var modalClose2 = new Hammer($('.modal-close')[1]);
    ...
    function modalDelete() {
        ...
        deleteBtn.on('tap', function() {
            if (!isDeleting) {
                isDeleting = true;
                $('.delete-btn-wrapper').hide(); 
                $('.modal .modal-main').append('<div class="deleting" style="top:' + top + 'px;"><img src="https://dn-'+ qiniu.bucket + '.qbox.me/img/loading_web.png" width="52"/></div>');
    ...
    $('.js-tp-like').on('click', function(e) {
        ...
        if (that.hasClass('tp-liked-black')) {
            ...
            thisPic.data('post').liked = false;
            thisPic.data('post').likes_count = thisPic.data('post').likes_count - 1;
        } else {
            that.addClass('tp-liked-black');
            likeNum.html(parseInt(likeNum.html())+1);
            addLike(tempPost);
    ...
    function deleteImage(post) {
     ...
     $('.tp-store-wrapper').on('click', function(e){
        e.preventDefault();
        if (e.offsetX < 300) {
            $(this).parent().animate({
                'margin-top': '-95px'
    ...
    if (isAndroid) {
        $('.tp-download-btn').attr('href', 'http://www.pgyer.com/RTIx');
    } else {
