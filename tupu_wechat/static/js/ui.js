...
//        var progressSize = $("<td/>");
//        progressSize.addClass("progressFileSize");

        var progressBarTd = $("<td/>");
        var progressBarBox = $("<div/>");
        progressBarBox.addClass('info clearfix');
...
//        progressBarPercent.text(fileSize);
        ...
        progressCancel.css('display','inline-block').addClass('progressCancel tp-icon tp-close');
...
FileProgress.prototype.removeProgress = function() {
//    this.fileProgressWrapper = $('#' + this.fileProgressID);
    if (!this.fileProgressWrapper.length) {
        jQuery('#' + this.fileProgressID).remove();
    }
}
...
//            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    ...
    tdProgress.removeClass().next().next('.status').hide();
...
FileProgress.prototype.setError = function() {
    this.fileProgressWrapper.find('.progress').css('background', '#E60012');
};
...
FileProgress.prototype.setImage = function(up, key) {
    ...
    var domain = up.getOption('domain');
    var url = domain + encodeURI(key);


    var progressNameTd = this.fileProgressWrapper.find('.progressName');
    var imageView = '?imageView2/1/w/80/h/80';
